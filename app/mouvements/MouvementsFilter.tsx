...
export function MouvementsFilter() {
  ...
  return (
    <form onSubmit={handleFilter} className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-end gap-3 mb-6 p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
      ...
      <div className="flex gap-2 w-full sm:w-auto pt-2 sm:pt-0">
        <button 
          type="submit" 
          className="flex-1 sm:flex-none px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors"
        >
          Appliquer
        </button>
        {(debut || fin) && (
          <button 
            type="button" 
            onClick={handleReset}
            className="flex-1 sm:flex-none px-4 py-2 bg-slate-100 text-slate-600 text-sm font-medium rounded-lg hover:bg-slate-200 transition-colors"
          >
            Réinitialiser
          </button>
        )}
